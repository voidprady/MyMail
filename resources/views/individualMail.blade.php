...
<div class="col-md-8 col-md-offset-2">
    <div class="panel panel-default">
        <div class="panel-heading">
          <p id="mailSubject">{{$mailDetails[0]['mail']['text']}}</p>
        </div>

        <div class="panel-body">
            <p>{{$mailDetails[0]['mail']['body']}} ---<span>{{$mailDetails[0]['mail']['user']['email']}}</span></p><br>
            @if($mailDetails[0]['mail']['attachment'] != null)
              <img src="data:image/jpg;base64,{{$mailDetails[0]['mail']['attachment']}}">
            @endif
            <button class="btn btn-target" onclick="trashReceivedMail({{$mailDetails[0]['mail']['id']}})">Trash it</button>
        </div>
        <p>----thread starts----</p>
...
<script>
function trashReceivedMail(id) {
  var data = {};
  data.id = id;
  $.ajax({
    type : 'POST',
    url : 'trashReceivedMail',
    data : data,
    success : function(){
      setTimeout(function(){
